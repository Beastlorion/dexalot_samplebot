...
abstract class AbstractBot {
  ...
  getEnvironment(envtype: string) {
    //mainnet, subnet
    return this.environments.find((item: any) => item.type === envtype && (item.env == "production-multi-avax" || item.env == "production-multi-subnet"));
  }
  ...
  async getTokenDetail(symbol: string): Promise<any> {
    return this.tokenDetails.find((item: any) => item.subnet_symbol === symbol && item.env == "production-multi-avax");
  }
